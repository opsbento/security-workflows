...
remaining_threshold="$(jq -r '.verification.remaining_threshold_findings // 0' "$result_file")"
...
- Remaining threshold findings: \`$remaining_threshold\`
- New threshold findings: \`$new_threshold\`
- New Critical findings: \`$new_critical\`
- Dependency files valid: \`$files_valid\`

<!-- remediation-core:$(jq -c '{ecosystem, dependency, dependencies, vulnerabilities, verification}' "$result_file") -->
BODY
